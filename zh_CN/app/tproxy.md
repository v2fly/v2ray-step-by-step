...
3. 手动配置 PC 的网络，将默认网关指向树莓派的地址即 `192.168.1.22`。centos7上默认开启了防火墙，可以先手动关闭，命令为：systemctl stop firewalld。如果不关闭防火墙，会出现能ping通，但是无法访问的现象。此时 PC 应当能正常上网（由于还没设置代理，“正常”是指可以上国内的网站）。
